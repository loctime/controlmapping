...
import { TrendingUp, FileCheck, AlertTriangle, CheckCircle2, Info, FileDown } from "lucide-react"
import type { AuditFile } from "@/domains/audit"
import { normalizeDate } from "@/utils/date"
import { NonComplianceList } from "./NonComplianceList"
import { pdf } from "@react-pdf/renderer"
import { PdfReport } from "./PdfReport"
import { preparePdfData } from "./pdfHelpers"
import {
  getCumplimientoPct,
  getHeaderNumber,
 ...
 * Las funciones están importadas desde dashboardHelpers.ts
 */
...
export function AuditDashboard({ auditFiles }: AuditDashboardProps) {
  ...
  const handleExportPdf = async () => {
    try {
      const pdfData = preparePdfData(auditFiles)
      const pdfDoc = <PdfReport data={pdfData} />
      const blob = await pdf(pdfDoc).toBlob()
      const url = URL.createObjectURL(blob)
      const link = document.createElement("a")
      link.href = url
      link.download = `reporte-auditorias-${new Date().toISOString().split("T")[0]}.pdf`
